...
class CBITS_LGBM_Module:
    TAKE_PROFIT_PERCENT = 0.75 # fixed 
    STOP_LOSS_PERCENT = 3.0 # fixed - we are pretty much assuming that there is no stop loss by setting it to be a large value 
    leverage = 1 # fixed 
    def __init__(self, symbol, bybit_credential, telegram_credential, lgbm_chkpt):
        ...
        self.lgbm = joblib.load(lgbm_chkpt)
    ...
    def execute_trade(self): 
        ...
        while True: 
            ...
            pred_class = self.lgbm.predict(x)[0] 
            pred_probs = self.lgbm.predict_proba(x)[0]
            pos_dict = {0:"Long", 1:"Short", 2:"Hold"} 
            self.send_message(f"Directional Prediction {pos_dict[pred_class]}, probability {np.max(pred_probs)}") 
            ...
            if pred_class == 0.0: 
                ...
                usdt = balances["result"]["USDT"]["available_balance"] 
                self.send_message("current cash status = " + str(usdt)) 
                self.send_message("skipping current iteration as we suspect low volatility") 
                move = 0 
            iteration += 1 
            if move == 0: 
                ...
                self.send_message("waiting for the next 8 hours \(^.^)/") 
                elapsed = time.time() - t0 
                time.sleep(60*60*8 - elapsed) 
