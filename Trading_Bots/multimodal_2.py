...
chart_df.drop(columns={"open", "high", "low", "close", "volume", "vwap"}, inplace=True) 
...
X_chart = torch.reshape(X_chart, (-1, X_chart.shape[2])) 
...
class BitTrader(nn.Module): 
    def __init__(self, chart_features, d_model, num_classes, plm, multi_dropout_r, multi_dropout_samples, chart_reduced_dim, news_reduced_dim): 
        super(BitTrader, self).__init__() 
        # intialize necessary variables  
        self.chart_features = chart_features 
        self.d_model = d_model 
        ...
        # chart encoder 
        self.batchnorm = nn.BatchNorm1d(self.chart_features) 
        self.chart_encoder = nn.Linear(self.chart_features, self.d_model)  
    ...
    def forward(self, x_chart, input_ids, attn_masks): 
        # encode chart data 
        x_chart = self.batchnorm(x_chart)
        x_chart = self.chart_encoder(x_chart)  
        x_chart = self.chart_multi_dropout(x_chart)  
...
model = BitTrader(chart_features=train_chart.shape[1],
                  d_model=256,
                  num_classes=3,
...
os.rename("Best_checkpoint.pt", f"Best_checkpoint_{best_loss}.pt")
