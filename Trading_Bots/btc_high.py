...
def preprocess_seq_data(chart_df, target="high"):
        targets = [] 
...
chart_df = preprocess_seq_data(chart_df, target="high") 
...
model = NeuralCLF(chart_features=X_train.shape[2], sequence_length=X_train.shape[1], d_model=256, num_classes=1, n_heads=8, num_encoders=6).cuda() 
...
for epoch_i in tqdm(range(epochs), desc="Epochs", position=0, leave=True, total=epochs):
        ...
        if avg_val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), f"HighEncoder.pt")
                
os.rename("HighEncoder.pt", f"HighEncoder_best_val_loss_{best_val_loss}.pt") 
