    ...
    def execute_trade(self):
        ...
        while True:
            ...
            if pred == 0.0:
                ...
                tp = 0.75
                stop_loss = stop_loss * (1-self.STOP_LOSS_PERCENT/100)
                ...
                while True:
                    try:
                        ...
                        self.send_message(e) 
                        self.send_message("error occurred when trying to open long position") 
                move = 1
            elif pred == 1.0:
                qty = self.get_position_size() 
                if qty == 0:
                ...
                while True:
                    try:
                        ...
                        self.send_message(e)
                        self.send_message("error occurred while opening short position") 
                move = -1
            elif pred == 2.0:
                qty = self.get_position_size()
                if qty == 0: 
                ...
                if np.abs(predicted_high) >= np.abs(predicted_low):
                    self.send_message("relying on price prediction as CBITS chose hold. Choosing long position")
                    max_bid, min_ask = self.get_best_bid_ask()
                    cur_price = (max_bid + min_ask) / 2.0
                    balances = self.exchange.fetch_balance({"coin":"USDT"})["info"]
                    usdt = balances["result"]["USDT"]["available_balance"]
                    ...
                    while True:
                        try:
                            ...
                            self.send_message(e) 
                            self.send_message("error occurred while opening long position") 
                    move = 1
                elif np.abs(predicted_high) < np.abs(predicted_low):
                    self.send_message("relying on price prediction as CBITS chose hold. Choosing short position")
                    max_bid, min_ask = self.get_best_bid_ask()
                    cur_price = (max_bid + min_ask) / 2.0
                    balances = self.exchange.fetch_balance({"coin":"USDT"})["info"]
                    usdt = balances["result"]["USDT"]["available_balance"]
...
if __name__ == '__main__':
    ...
    trader = CBITS_v3(
        ...
        cbits_zip_dir="tabnet_roberta_full_.zip",
        tabnet_high_dir="reg_high_chart_only_seed0_.zip",
        tabnet_low_dir="reg_low_chart_only_seed0_.zip"
    )

# start trade (1->5->9->1->... 사이클)
time.sleep(60*66)
