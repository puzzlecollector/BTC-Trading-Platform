...
for i in tqdm(range(1, 10932), position=0, leave=True, desc="scraping content from tokenpost"):
    ...
    try:
        ...
        unique_links = np.unique(links)[::-1] # reverse order
...
df.to_csv("tokenpost_test.csv", index=False)
